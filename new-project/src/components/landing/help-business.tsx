...
export function HelpBusiness() {
  return (
    <section id="help" aria-labelledby="help-heading" className="bg-background py-16 sm:py-24">
      <Container>
        <div className="max-w-3xl">
          <p className="kicker">How we help your business</p>
          <h2
            id="help-heading"
        ...
        <ul className="mt-12 grid gap-6 lg:grid-cols-2">
          {problems.map((item) => (
            <li key={item.title} className="paper-card rounded-sm border border-border p-6 sm:p-8">
        ...
        <ol className="mt-8 space-y-6">
          {helpPoints.map((item, index) => (
            <li key={item.title} className="grid gap-3 border-t border-border pt-6 md:grid-cols-[4rem_minmax(0,1fr)]">
